...
void Key(void)
{	
	int Position=2080; //Target position The original position of the motor is 10000. One turn is 1040. It is related to the accuracy of the encoder. The default is that the swing lever Z-turn is one turn and the output is 1040 jump edges.
  ...
  if(tmp==2)flag=2;//--
	...
	if(flag==1) //Pendulum movement clockwise
	{
		Position_Zero++;
		count++;	
		if(count==Position) 	flag=0,count=0;
	}	
		if(flag==2) //Pendulum counterclockwise movement
	{
		Position_Zero--;
		count++;	
		if(count==Position) 	flag=0,count=0;
...
{
	    u8 temp; 
			if(1==Flag_Stop||Angle_Balance<(ZHONGZHI-800)||Angle_Balance>(ZHONGZHI+800)||voltage<1110) //Battery voltage is too low, turn off the motor
			{	      
      Flag_Stop=1;				
      temp=1;                                            
			AIN1=0;                                            
